...
let PTOLPriceConfigForm = React.createClass({
  ...
  propTypes: {
    ...
    nLabelPortCaptainOutlayDays: PropTypes.string,
  ...
  render() {
    ...
    return (
      <div style = {this.style('root')}>
        <TextFieldUnit
          ref = 'portCaptainOutlay'
          key = 'portCaptainOutlay'
          style= {this.style('textFieldUnit')}
          defaultValue= {priceConfig ? priceConfig['CTPTCNOY|COUNT|value'] == 0 ? '' : priceConfig['CTPTCNOY|COUNT|value'] : ''}
          floatingLabelText= {this.t('nLabelPortCaptainOutlayDays')}
          unitLabelText={this.t('nLabelDaysStayUnit')}
  ...
  _getProductConfig(){
    ...
    let products = _.map(productConfig.products,product=>{
      ...
      if(CTM_CODE===productCode && this.refs.portCaptainOutlay){
        product.priceConfig['CTPTCNOY|COUNT|value'] = parseInt(this.refs.portCaptainOutlay.getValue());
      }
      return product;
    });
    productConfig.products = products;
  ...
  _getPriceConfig(){
    ...
    _.forEach(productConfig.products,product=>{
      if(CTM_CODE===product.product.code){
...
module.exports = PTOLPriceConfigForm;
