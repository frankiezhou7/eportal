...
const AGM_INSPECTION = 'PTAGMINPECN';
const UNDER_WATER_INSPECTION = 'PTUDWTERINPECN';
const PRE_PURCHASE_INSPECTION = 'PTPPSEINPECN';

let PTISPConfigForm = React.createClass({
  ...
  renderSubProductChild(config, subProduct){
    ...
    let acceptVideo = subProductCode === AGM_INSPECTION ? false: true;
    return (
      <div>
        <TextField
  ...
  renderSubProduct(select,config,subProduct){
    return (
      ...
      >
        ...
        >
          {this.renderSubProductChild(config.toJS(),subProduct)}
        </Flexible>
      </div>
    );
  },
  ...
  _getProductConfig(){
    ...
    let products = _.map(productConfig.products,product=>{
      ...
      if(this.refs['flex_'+productCode]){
        product.select = this.refs['flex_'+productCode].isSelected();
      }
      if(productCode === AGM_INSPECTION && this.props.ship){
        product.priceConfig['CTAGMINSPNCG|GRT|value'] = this.props.ship.getGRT();
      }
...
module.exports = PTISPConfigForm;
